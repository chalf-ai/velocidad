...
import {
  type AgregadoCalidadCierre,
  type TipoHuerfano,
  rankingPeoresCierre,
} from "@/lib/historico/vista-derivados";
import type { ConflictoKind, EntradaConsolidada } from "@/lib/historico/cruce-roma-actas";
...
interface Props {
  ...
  onFoco: (v: FocoCalidadCierre | null) => void;
}
...
export function EjeCalidadCierreInline({ data, filas, foco, onFoco }: Props) {
  const isFocoEstado = (e: CalidadCierre | "no_evaluable") => foco?.tipo === "estado" && foco.valor === e;
  ...
  const ranking = useMemo(
    () => ({
      sucursal: rankingPeoresCierre(filas, "sucursal"),
      marca: rankingPeoresCierre(filas, "marca"),
      vendedor: rankingPeoresCierre(filas, "vendedor"),
    }),
    [filas],
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <CardBody className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[15px] font-semibold text-[--color-fg]">Calidad de cierre</h3>
            </div>
            {foco && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {(["correcto", "huerfano", "inconsistente", "no_evaluable"] as const).map((e) => (
              <Stat
                ...
                size="md"
                as="button"
                onClick={() => onFoco(isFocoEstado(e) ? null : { tipo: "estado", valor: e })}
          ...
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[--color-fg-muted] font-medium mb-2">
              Huérfanos por tipo · sobre {fmtNum(data.totalHuerfanos)} · click = drill
            </div>
            <div className="flex flex-wrap gap-2">
              {(["tipo1", "tipo2", "tipo3", "tipo4", "otro"] as TipoHuerfano[]).map((t) => (
                <button
                  key={t}
                  onClick={() => onFoco(isFocoHuer(t) ? null : { tipo: "huerfano", valor: t })}
                  className={cn(
                ...
                >
                  <Badge tone="warning" size="xs" dot>{HUERFANO_LABEL[t]}</Badge>
          ...
          <div>
            <div className="text-[11px] uppercase tracking-wider text-[--color-fg-muted] font-medium mb-2">
              Inconsistencias por conflicto material · sobre {fmtNum(data.totalInconsistentes)} · click = drill
            </div>
            <div className="flex flex-wrap gap-2">
              {(Object.keys(data.inconsistentesPorConflicto) as ConflictoKind[])
                .filter((k) => data.inconsistentesPorConflicto[k] > 0)
                .sort((a, b) => data.inconsistentesPorConflicto[b] - data.inconsistentesPorConflicto[a])
                .map((k) => (
                  <button
                    key={k}
                    onClick={() => onFoco(isFocoConf(k) ? null : { tipo: "conflicto", valor: k })}
                    className={cn(
                      "inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11.5px] font-medium",
                      "ring-1 ring-inset transition",
                      isFocoConf(k)
                  ...
                  >
                    <Badge tone="danger" size="xs" dot>{KIND_LABEL[k]}</Badge>
                    <span className="text-[--color-fg-muted]">{fmtNum(data.inconsistentesPorConflicto[k])}</span>
                  </button>
                ))}
      ...
      <RankingAccionableCard
        titulo="Top peores por calidad de cierre"
        descripcion="Ranking ordenado por mayor % de cierre problemático (huérfano + inconsistente) sobre entregados."
        unidad="porcentaje"
