...
import {
  type AgregadoVelocidad,
  type TimelineProceso,
  type TramoId,
  procesoDeCuello,
...
import { TimelineHorizontalProceso } from "@/components/historico/TimelineHorizontalProceso";
import { RankingAccionableCard } from "@/components/historico/RankingAccionableCard";

interface Props {
  data: AgregadoVelocidad;
  filas: EntradaConsolidada[]; // filtradas — para el ranking
  foco: FocoVelocidad | null;
  onFoco: (v: FocoVelocidad | null) => void;
  timeline: TimelineProceso | null;
...
export function EjeVelocidadInline({
  data,
  filas,
  foco,
  onFoco,
  ...
  const isFocoCuello = (c: CuelloPrincipal) => foco?.tipo === "cuello" && foco.valor === c;
  const isFocoBucket = (b: BucketVelocidad) => foco?.tipo === "bucket" && foco.valor === b;

  // Cuello dominante del eje
  const cuelloDominante = data.distribucionCuello[0];
  const cuelloOperacional =
    foco?.tipo === "cuello" ? procesoDeCuello(foco.valor as CuelloPrincipal) : null;

  // Ranking peores por mediana
  const ranking = useMemo(
    () => ({
      sucursal: rankingPeoresVelocidad(filas, "sucursal"),
      marca: rankingPeoresVelocidad(filas, "marca"),
      vendedor: rankingPeoresVelocidad(filas, "vendedor"),
    }),
    [filas],
  );
  ...
  return (
    <div className="space-y-4">
        ...
        <CardBody className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[15px] font-semibold text-[--color-fg]">Velocidad operacional</h3>
            </div>
            {foco && (
          ...
          {/* KPIs compactos del eje */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Stat label="Total casos" value={fmtNum(data.totalCasos)} size="md" />
            <Stat
              label="Mediana días"
              value={data.diasTotales.mediana ?? "—"}
              sub={`n=${fmtNum(data.diasTotales.nConDatos)} con extremos`}
              size="md"
            />
            <Stat label="Promedio" value={data.diasTotales.promedio ?? "—"} size="md" />
            <Stat label="p90" value={data.diasTotales.p90 ?? "—"} size="md" />
          </div>

          {/* Cuello dominante destacado */}
          {cuelloDominante && (
            <div className="text-[11.5px] text-[--color-fg-muted]">
              Cuello dominante:{" "}
              <span className="text-[--color-fg] font-medium">{cuelloDominante.cuello}</span>{" "}
              {fmtNum(cuelloDominante.cantidad)} ({cuelloDominante.pct}%)
            </div>
          )}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {data.distribucionCuello.map((d) => (
                <button
                  key={d.cuello}
                  onClick={() => onFoco(isFocoCuello(d.cuello) ? null : { tipo: "cuello", valor: d.cuello })}
                  className={cn(
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {(Object.keys(data.distribucionVelocidad) as BucketVelocidad[]).map((b) => (
                <button
                  key={b}
                  onClick={() => onFoco(isFocoBucket(b) ? null : { tipo: "bucket", valor: b })}
                  className={cn(
                    "inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-[11.5px] font-medium",
                    "ring-1 ring-inset transition",
                    isFocoBucket(b)
      ...
      {/* Timeline horizontal solo cuando foco es cuello operacional */}
      {cuelloOperacional && timeline && foco?.tipo === "cuello" && (
        <TimelineHorizontalProceso
          data={timeline}
          focoTramo={focoTramo}
          onFocoTramo={onFocoTramo}
          cuelloLabel={foco.valor as CuelloPrincipal}
        />
      )}

      <RankingAccionableCard
        titulo="Top peores por velocidad"
        descripcion="Ranking ordenado por mayor mediana de días totales del ciclo (peor)."
        unidad="dias"
